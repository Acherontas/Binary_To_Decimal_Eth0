#ifndef MASKS_H
#define MASKS_H

#include <string>
#include <string.h>
#include <iostream>
using namespace std;

class masks
{
    public:
        masks();
        virtual ~masks();
        masks(const masks& other);
        masks& operator=(const masks& other);
        string maskone[100]={
        "0",
        "10000000.00000000.00000000.00000000","1",
        "11000000.00000000.00000000.00000000","2",
        "11100000.00000000.00000000.00000000","3",
        "11110000.00000000.00000000.00000000","4",
        "11111000.00000000.00000000.00000000","5",
        "11111100.00000000.00000000.00000000","6",
        "11111110.00000000.00000000.00000000","7",
        "11111111.00000000.00000000.00000000","8",
        "11111111.10000000.00000000.00000000","9",
        "11111111.11000000.00000000.00000000","10",
        "11111111.11100000.00000000.00000000","11",
        "11111111.11110000.00000000.00000000","12",
        "11111111.11111000.00000000.00000000","13",
        "11111111.11111100.00000000.00000000","14",
        "11111111.11111110.00000000.00000000","15",
        "11111111.11111111.00000000.00000000","16",
        "11111111.11111111.10000000.00000000","17",
        "11111111.11111111.11000000.00000000","18",
        "11111111.11111111.11100000.00000000","19",
        "11111111.11111111.11110000.00000000","20",
        "11111111.11111111.11111000.00000000","21",
        "11111111.11111111.11111100.00000000","22",
        "11111111.11111111.11111110.00000000","23",
        "11111111.11111111.11111111.00000000","24",
        "11111111.11111111.11111111.10000000","25",
        "11111111.11111111.11111111.11000000","26",
        "11111111.11111111.11111111.11100000","27",
        "11111111.11111111.11111111.11110000","28",
        "11111111.11111111.11111111.11111000","29",
        "11111111.11111111.11111111.11111100","30",
        "11111111.11111111.11111111.11111110","31",
        "11111111.11111111.11111111.11111111","32"};
        string mask_users[94]={
        "0",
        "2147483646","31","1",
        "1073741822","30","2",
        "536870910","29","3",
        "268435454","28","4",
        "134217726","27","5",
        "67108862","26","6",
        "33554430","25","7",
        "16777214","24","8",
        "8388606","23","9",
        "4194302","22","10",
        "2097150","21","11",
        "1048574","20","12",
        "524286","19","13",
        "262141","18","14",
        "131070","17","15",
        "65534","16","16",
        "32766","15","17",
        "16382","14","18",
        "8190","13","19",
        "4094","12","20",
        "2046","11","21",
        "1022","10","22",
        "510","9","23",
        "254","8","24",
        "126","7","25",
        "62","6","26",
        "30","5","27",
        "14","4","28",
        "6","3","29",
        "2","2","30",
        "0","1","31"
        };
        string retrieve_mask;
        string fnli;
        string masks_conce[67]={
        "0",
        "128.0.0.0","1",
        "192.0.0.0","2",
        "224.0.0.0","3",
        "240.0.0.0","4",
        "248.0.0.0","5",
        "252.0.0.0","6",
        "254.0.0.0","7",
        "255.0.0.0","8",
        "255.128.0.0","9",
        "255.192.0.0","10",
        "255.224.0.0","11",
        "255.240.0.0","12",
        "255.248.0.0","13",
        "255.252.0.0","14",
        "255.254.0.0","15",
        "255.255.0.0","16",
        "255.255.128.0","17",
        "255.255.192.0","18",
        "255.255.224.0","19",
        "255.255.240.0","20",
        "255.255.248.0","21",
        "255.255.252.0","22",
        "255.255.254.0","23",
        "255.255.255.0","24",
        "255.255.255.128","25",
        "255.255.255.192","26",
        "255.255.255.224","27",
        "255.255.255.240","28",
        "255.255.255.248","29",
        "255.255.255.252","30",
        "255.255.255.254","31",
        "255.255.255.255","32"
        };
        string vlsm_mask;
        string vlsm_mask_bd;
        int one_to_one(string user_ip_addr,string choise_of_mask);
        int one_to_many(string user_ip_addr);
        int one_to_oneip(string user_ip_addrone,string user_ip_addrtwo);
        string fu;
        string rvm;
        string lu;
        string bdc;
        int first_user(string gw);
        int last_user(string fu,string rmv); //last_user + broadcast_user
        int reverse_mask(string gw);
        string return_mask(string mskk);
        string return_users(string users);
        int expose_mask=0;
        string expd_mask;
        string nxt_neti;
        string mygw;
        string nxt_network(int expose_maske,string mgw);
        string fnext_net;
        int check_ip_addr(string ip_to_check);
        int return_check=0;
        int x_ones=0;
        int z_zeros=0;
        int sum=0;
        char gen_char='\0';
        int arip[4];
        string aripp;
        string sof_fir;
        string sof_fir_as_bn;
    protected:
    private:
};

#endif // MASKS_H
